...
class TD3Agent:
    """
    TD3 agent encapsulating Actor + twin Critic networks, their target
    networks, the OU noise process, and the update logic. Adds three TD3
    refinements on top of DDPG: twin critics with min-Q target, clipped
    Gaussian noise added to the target action (target policy smoothing),
    and delayed policy / target updates that fire every policy_delay
    critic steps.
    """

    def __init__(self, n_states, n_actions, action_bound, device,
                 actor_lr=2e-4, critic_lr=1e-3, gamma=0.99, tau=0.002,
                 policy_delay=2, target_noise_sigma=0.2, target_noise_clip=0.5):
        self.device = device
        self.gamma = gamma
        self.tau = tau
        self.action_bound = action_bound
        ...
        # Main networks: one actor + two independent critics (Q1, Q2)
        self.actor   = Actor(n_states, n_actions, action_bound).to(device)
        self.critic1 = Critic(n_states, n_actions).to(device)
        self.critic2 = Critic(n_states, n_actions).to(device)

        # Target networks initialised to the same weights
        self.actor_target   = Actor(n_states, n_actions, action_bound).to(device)
        self.critic1_target = Critic(n_states, n_actions).to(device)
        self.critic2_target = Critic(n_states, n_actions).to(device)
    ...
    def update(self, buffer, batch_size):
        ...
        with torch.no_grad():
            noise = (torch.randn_like(actions) * self.target_noise_sigma).clamp(
                -self.target_noise_clip, self.target_noise_clip)
            next_actions = (self.actor_target(next_states) + noise).clamp(
                -self.action_bound, self.action_bound)

            # Twin critics: take the min of the two target Q-values
            target_q1 = self.critic1_target(next_states, next_actions)
            target_q2 = self.critic2_target(next_states, next_actions)
            target_q  = torch.min(target_q1, target_q2)
            target_q  = rewards + self.gamma * (1 - dones) * target_q

        # Critic update: MSE against the shared Bellman target for each Q-network
        critic1_loss = nn.MSELoss()(self.critic1(states, actions), target_q)
        self.critic1_optimizer.zero_grad()
        critic1_loss.backward()
        torch.nn.utils.clip_grad_norm_(self.critic1.parameters(), 1.0)
        self.critic1_optimizer.step()

        critic2_loss = nn.MSELoss()(self.critic2(states, actions), target_q)
        self.critic2_optimizer.zero_grad()
        critic2_loss.backward()
        torch.nn.utils.clip_grad_norm_(self.critic2.parameters(), 1.0)
        self.critic2_optimizer.step()

        # Delayed policy update: actor and target networks update less often
        if self.total_it % self.policy_delay == 0:
            # Actor uses only Q1 to keep the policy gradient smooth
            ...
            # Soft update all three target networks: θ_target ← τθ + (1-τ)θ_target
...
def plot_results(episode_rewards, episode_lengths, show_result=False):
    ...
    if show_result:
        ...
        plt.savefig("results/td3_results.png", dpi=150, bbox_inches="tight")
...
if __name__ == "__main__":
    ...
    MODEL_SAVE_DIR  = "td3_models"
    ACTOR_PATH      = os.path.join(MODEL_SAVE_DIR, "actor_model.pth")
    CRITIC1_PATH    = os.path.join(MODEL_SAVE_DIR, "critic1_model.pth")
    CRITIC2_PATH    = os.path.join(MODEL_SAVE_DIR, "critic2_model.pth")
    ...
    ###########################################################################
    print("=" * 60)
    print("TD3 Mountain Car - Mode Selection")
    print("=" * 60)
    ...
    if MODE == "DEMO":
        ...
        agent = TD3Agent(n_states, n_actions, action_bound, device)
